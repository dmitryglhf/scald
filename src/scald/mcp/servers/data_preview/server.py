...
@mcp.tool(
    description="Inspect CSV file and return metadata: shape, column names, data types, missing value counts, and memory usage. Does not load full dataset."
)
async def inspect_csv(
    ...
    logger.info(f"[MCP:data_preview] inspect_csv: {file_path}")
    try:
        path = Path(file_path)
        if not path.exists():
        ...
        logger.info(
            f"[MCP:data_preview] Inspected CSV: {df.height} rows, {df.width} columns, "
            f"{sum(info.missing_counts.values())} missing values"
        )
        return InspectResult(success=True, info=info, error=None)

    except Exception as e:
        logger.error(f"[MCP:data_preview] Failed to inspect CSV: {e}")
        return InspectResult(success=False, info=None, error=str(e))


@mcp.tool(
    description="Preview first N rows of CSV file as list of dictionaries. Useful for examining actual data values and patterns."
)
async def preview_csv(
    ...
    logger.info(f"[MCP:data_preview] preview_csv: {file_path}, n_rows={n_rows}")
    try:
        if n_rows <= 0:
        ...
        logger.info(f"[MCP:data_preview] Previewed {df.height} rows from CSV")
        return {
            "success": True,
            "preview": df.to_dicts(),
            "columns": df.columns,
        ...
        logger.error(f"[MCP:data_preview] Failed to preview CSV: {e}")
